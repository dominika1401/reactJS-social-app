...
const Post = (props) => {
    ...
    const [likesCount, setLikeCount] = useState(props.post.likes.length)
    const [delteModelVisible, setDelteModelVisible] = useState(false);
    ...
    const likePost = (id, isLiked) => {
        axios.post('https://akademia108.pl/api/social-app/post/' + (isLiked ? 'dislike' : 'like'), {
            ...
            setLikeCount(likesCount + (isLiked ? -1 : 1))
    ...
    return (
        <div className="post">
            ...
            <div className="postData">
                ...
                <div className="likes">
                    {props.user?.username === props.post.user.username && (<button className='btn' onClick={() => setDelteModelVisible(true)}>Delete</button>)}
